...
p = 37
q = 7
if (not checkPrime(p,q)):
  p = 31
  q = 23

en = b'an example of a message from alice to bob'
nRabin = p*q
resSig, resU = root(en,p,q)
encryp = int.from_bytes(en,'big')
sig2 = resSig*resSig
condVerified = (h(encryp,resU)) % nRabin == ((sig2)% nRabin)
print("Verify that the message was received from the real sender -->", condVerified)
if condVerified:

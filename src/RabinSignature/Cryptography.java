...
class Cryptography {
    ...
    public static BigInteger[] decrypt(BigInteger c, BigInteger p, BigInteger q)
    {
        ...
        BigInteger d4 = (y_q.multiply(q).multiply(p2)).mod(N);
 ...
 class RabinCryptosystem {
    public static void main(String[] args)
    {
        ...
        BigInteger n = key[0]; //i guess its the random number u
        BigInteger p = key[1];
        BigInteger q = key[2];
        ...
        BigInteger m = new BigInteger(s.getBytes(Charset.forName("ascii")));//message in bytes.
